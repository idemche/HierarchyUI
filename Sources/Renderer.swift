...
public final class NavigationHierarchyRenderer {
    ...
    private lazy var navigator: Navigator = Navigator(onPush: { [weak self] key in
        ...
        guard let route = nextReplacerByKey ?? nextReplacerRandomized else { return assertionFailure() }
        ...
        let view = route.rootViewProvider().environmentObject(self.navigator)
        window.rootViewController = EventNotifyingHostingController(key: route.key, rootView: view)
